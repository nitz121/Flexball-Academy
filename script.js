const levels = [
  { team: "Real Madrid", formation: "The Defensive Line", instruction: "Protect the bottom goal. Put the four defenders side by side, centred directly in front of the goal.", players: ["Dani Carjaval", "Sergio Ramos", "Pepe", "Marcelo"], answer: { flexDirection: "row", justifyContent: "center", alignItems: "flex-end", flexWrap: "nowrap" } },
  ...
  { team: "Liverpool", formation: "High Press", instruction: "Press near the opponent's top goal. Place the attackers side by side in the top-left corner of the pitch.", players: ["Mohamed Salah", "Sadio Mané", "Roberto Firmino"], answer: { flexDirection: "row", justifyContent: "flex-start", alignItems: "flex-start", flexWrap: "nowrap" }, startingValues: { flexDirection: "row", justifyContent: "center", alignItems: "flex-start", flexWrap: "nowrap" } },
  ...
  { team: "Juventus", formation: "Right-Flank Attack", instruction: "Build the attack down the right flank. Create a vertical column along the right edge with equal spacing before, between, and after every player.", players: ["Gianluigi Buffon", "Alessandro Del Piero", "Pavel Nedvěd", "David Trezeguet"], answer: { flexDirection: "column", justifyContent: "space-evenly", alignItems: "flex-end", flexWrap: "nowrap" } },
...
const storageKey = "flexball-progress-v2";
...
function startingValues(levelIndex) { return levels[levelIndex].startingValues || defaultValues(); }
...
function celebrate() {
  ...
  field.classList.remove("celebrate");
  field.querySelector(".celebration-overlay")?.remove();
  const overlay = document.createElement("div");
  overlay.className = "celebration-overlay";
  overlay.setAttribute("aria-hidden", "true");
  overlay.innerHTML = `<strong>GOAL! 🎉</strong>${Array.from({ length: 18 }, (_, index) => `<i style="--x:${(index * 53) % 100}%; --delay:${(index % 6) * 55}ms"></i>`).join("")}`;
  field.append(overlay);
  void field.offsetWidth;
  field.classList.add("celebrate");
...
function renderLevel() {
  ...
  const values = progress.selections?.[progress.currentLevel] || startingValues(progress.currentLevel);
...
resetButton.addEventListener("click", () => { progress.selections ??= {}; progress.selections[progress.currentLevel] = startingValues(progress.currentLevel); renderLevel(); });
checkButton.addEventListener("click", () => {
  if (progress.completed[progress.currentLevel]) {
    feedback.textContent = "This level is already complete. Choose another unlocked level from the Training Ground.";
